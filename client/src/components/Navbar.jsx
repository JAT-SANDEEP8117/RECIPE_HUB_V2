        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden lg:flex items-center gap-4">
              <Link to="/about" className="text-sm font-medium text-slate-300 hover:text-orange-400 transition-colors">
                About
              </Link>
              {user && user.role === 'admin' && (
                <Link to="/admin" className="text-sm font-medium text-slate-300 hover:text-orange-400 transition-colors">
                  Admin
                </Link>
              )}
              {onFilter && ['Indian', 'Italian', 'Chinese', 'Others'].map((cat) => (
                <button
                  key={cat}
            ...
            <div className="flex items-center gap-3 border-l border-slate-700 pl-6">
              <Link to="/about" className="lg:hidden text-sm font-medium text-slate-300 hover:text-orange-400 transition-colors">
                About
              </Link>
              {user && user.role === 'admin' && (
                <Link to="/admin" className="lg:hidden text-sm font-medium text-slate-300 hover:text-orange-400 transition-colors">
                  Admin
                </Link>
              )}
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-orange-400 hidden sm:inline">Hello, {user.name.split(' ')[0]}</span>
                  <button onClick={logout} className="text-sm font-semibold text-slate-300 hover:text-white transition-colors cursor-pointer">
                    Logout
                  </button>
                </div>
              ) : (
